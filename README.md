# CLIProject
Command-line interface system built in Java with MySQL database integration. Supports dynamic command management, authenticated user access, and secure client-server communication.

- All commands are executed on the server, not the client.
- Only authenticated users can connect and run commands.
- Commands are stored in a MySQL database, allowing dynamic updates without modifying Java code.
...
# TECHNOLOGIES 
1. MySQL database for data management
